...
const BASE_URL = process.env.BASE_URL || CONFIG.BASE_URL || 'https://starflow.uk';
...
// Config
// BASE_URL is defined via CONFIG/env in remote-enabled build
...
function renderTemplate(templateName, slots) {
  ...
  // Minimal slot replacement {{KEY=...}} or {{KEY}} markers
  // Replace assignment-style first
  ...
  Object.keys(slots).forEach(k => {
    ...
    tpl = tpl.replace(re, slots[k]);
  });
  // Handle layout injection if template starts with {{#layout}}
  if (tpl.startsWith('{{#layout}}')) {
    // Extract inline meta assignments
    const meta = {};
    const assigns = [...tpl.matchAll(/{{([A-Z0-9_]+)=(.*?)}}/gs)];
    assigns.forEach((m) => {
      meta[m[1]] = m[2];
    });
    // Remove markers and content wrapper
    let content = tpl
      .replace('{{#layout}}', '')
      .replace(/{{[A-Z0-9_]+=(.*?)}}/gs, '')
      .trim();
    return renderLayout(content, meta);
  }
  return tpl;
}
...
function councilTermRows(council) {
  return termDates.filter(t => t.council === council);
}
function councilBinsRow(council) {
  return bins.find(b => b.council === council);
}
function townVenues(town, types) {
  return venues.filter(v => v.town === town && types.includes(v.type));
}
...
function townPools(town) {
  return poolsFamily.filter(p => p.town === town);
}
function townLatePharmacies(town) {
  return pharmacyLate.filter(p => p.town === town);
}

(async () => {
await loadAll();
placesByTown = Object.fromEntries(places.map(p => [p.town, p]));
townsByCounty = places.reduce((acc, p) => { acc[p.county] = acc[p.county] || []; acc[p.county].push(p); return acc; }, {});
...
// UK index grouped by county
const countyBlocks = Object.keys(townsByCounty).sort().map(county => {
  const links = townsByCounty[county].map(p => {
    const u = `/${p.town_slug || slugify(p.town)}/`;
    return `<a href="${u}">${p.town}</a>`;
  }).join(' · ');
...
places.forEach(p => {
  ...
  const binSummary = binRow ? `<p>${binRow.general_rules || ''}</p><p><a href="${binRow.link_to_checker}" target="_blank" rel="nofollow">Council checker</a></p>` : 'Add bin info';
  ...
  const urgent = [
    ...
    ...townLatePharmacies(town).slice(0,2).map(p => `${p.name} <span class="muted">late ${p.close_time}</span>`)
  ...
  function sectionPage(title, intro, rowsHtml, descr) {
    const html = renderTemplate('section.html', {
      TITLE: title,
      TOWN: town,
  ...
  // Term dates section
  {
    ...
    const rowsHtml = rows.length ? table(['Term','Start','End','Inset Days'], rows.map(r => [r.term_name, r.start_date, r.end_date, (r.inset_dates||'').replace(/;/g, ', ')])) : '<p>Add term_dates.csv rows for this council.</p>';
  ...
  // Bins section
  {
    ...
    const rowsHtml = b ? (`<p><strong>General rules:</strong> ${b.general_rules || ''}</p>
  ...
  // Baby groups
  {
    ...
    const rowsHtml = items.length ? list(items.map(v => `<strong>${v.name}</strong><br><span class="muted">${v.address || ''}</span><br>${v.opening_hours || ''}<br><a href="${v.url}" target="_blank" rel="nofollow">${v.url}</a>`)) : '<p>Add baby groups to venues.csv (type=baby_group)</p>';
  ...
  // Soft play & parks
  {
    ...
    const rowsHtml = items.length ? list(items.map(v => `<strong>${v.name}</strong> <span class="badge">${v.type}</span><br><span class="muted">${v.address || ''}</span><br>${v.price_note || ''}<br><a href="${v.url}" target="_blank" rel="nofollow">${v.url}</a>`)) : '<p>Add soft play and parks to venues.csv (type=soft_play or park)</p>';
  ...
  // Pools family times
  {
    ...
    const rowsHtml = items.length ? table(['Session','Day','Start','End','Price','Link'], items.map(s => [s.session_name, s.day_of_week, s.start_time, s.end_time, s.price_note || '', s.link ? `<a href="${s.link}" target="_blank" rel="nofollow">Timetable</a>` : '' ])) : '<p>Add pools_family.csv rows for this town.</p>';
  ...
  // Urgent care & pharmacies
  {
    ...
    if (urgentCare.length) {
      parts.push('<h3>Urgent care</h3>' + list(urgentCare.map(v => `<strong>${v.name}</strong><br>${v.address || ''}<br><a href="${v.url}" target="_blank" rel="nofollow">${v.url}</a>`)));
    }
    if (latePh.length) {
      parts.push('<h3>Late pharmacies</h3>' + list(latePh.map(p => `<strong>${p.name}</strong> · open late ${p.days_open_late} until ${p.close_time}<br>${p.address || ''}<br><a href="${p.nhs_url}" target="_blank" rel="nofollow">NHS listing</a>`)));
    }
  ...
  // Free this weekend
  {
    ...
    const rowsHtml = items.length ? list(items.map(e => `<strong>${e.date}</strong> — <a href="${e.url}" target="_blank" rel="nofollow">${e.title}</a><br><span class="muted">${e.venue_name || ''} · ${e.venue_address || ''}</span>`)) : '<p>No free events added for the next 7 days.</p>';
